...
| **Theremin** | `theremin` | 4x3 | `pitch-in` (cv), `vol-in` (cv), `gate-in` (gate) | `out` (audio), `pitch-cv` (cv), `gate` (gate), `vol` (cv) | `frequency`=440, `volume`=0, `touch`=0, `waveform`=0, `vibratoRate`=5, `vibratoDepth`=0, `tremoloRate`=5, `tremoloDepth`=0, `tone`=0.6, `glide`=0.05, `level`=1, `scaleLock`=false, `scale`=2, `root`=0, `loFreq`=130.81, `hiFreq`=1046.5 |
