...
const drumConfigs: Record<string, DrumKnobConfig[]> = {
  ...
  // TR-909 Drums — knob params/ranges aligned to the actual DSP (apply_param + voice clamps).
  // Decay is in SECONDS (the voice DSP reads it as a duration); dead UI-only params removed.
  '909-kick': [
    { label: 'Tune', param: 'tune', min: 30, max: 120, step: 1, defaultVal: 55, unit: 'Hz', format: formatInt },
    { label: 'Click', param: 'attack', min: 0, max: 1, step: 0.01, defaultVal: 0.5, format: formatPercent },
    { label: 'Decay', param: 'decay', min: 0.1, max: 2.0, step: 0.01, defaultVal: 0.5, unit: 's', format: (v) => v.toFixed(2) },
  ...
  '909-snare': [
    ...
    { label: 'Decay', param: 'decay', min: 0.05, max: 1.0, step: 0.01, defaultVal: 0.3, unit: 's', format: (v) => v.toFixed(2) },
  ],
  '909-hihat': [
    { label: 'Tune', param: 'tune', min: 0.5, max: 2, step: 0.01, defaultVal: 1.0, format: (v) => `×${v.toFixed(2)}` },
    { label: 'Decay', param: 'decay', min: 0.02, max: 1.5, step: 0.01, defaultVal: 0.2, unit: 's', format: (v) => v.toFixed(2) },
    { label: 'Tone', param: 'tone', min: 0, max: 1, step: 0.01, defaultVal: 0.5, format: formatPercent },
    { label: 'Open', param: 'open', min: 0, max: 1, step: 0.01, defaultVal: 0.0, format: formatPercent },
  ],
  '909-clap': [
    { label: 'Tone', param: 'tone', min: 0, max: 1, step: 0.01, defaultVal: 0.5, format: formatPercent },
    { label: 'Decay', param: 'decay', min: 0.1, max: 1.0, step: 0.01, defaultVal: 0.4, unit: 's', format: (v) => v.toFixed(2) },
  ],
  '909-tom': [
    { label: 'Tune', param: 'tune', min: 60, max: 300, step: 1, defaultVal: 120, unit: 'Hz', format: formatInt },
    { label: 'Decay', param: 'decay', min: 0.1, max: 1.5, step: 0.01, defaultVal: 0.4, unit: 's', format: (v) => v.toFixed(2) },
  ],
  '909-rimshot': [
    { label: 'Tune', param: 'tune', min: 200, max: 600, step: 1, defaultVal: 400, unit: 'Hz', format: formatInt },
  ],
  '909-crash': [
    { label: 'Tune', param: 'tune', min: 0.5, max: 2, step: 0.01, defaultVal: 1.0, format: (v) => `×${v.toFixed(2)}` },
