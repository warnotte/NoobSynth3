...
| `scripts/bench/` | voir `scripts/bench/README.md` | **Banc de recréation** : 17 instruments de mesure pour refaire un son ou un morceau entier sur des chiffres (référence → mesure → patch → `render_graph` → re-mesure du rendu avec les MÊMES outils). Carte mesure par mesure (`structure.mjs`), piano-roll polyphonique (`pianoroll.mjs`, à lancer AVANT `lead-track.mjs` qui aplatit la polyphonie), ligne de basse entre les kicks, coup de batterie à la loupe (`hit.mjs`), modulation rapide d'une note (`purr.mjs`), extinction/écho (`decay.mjs`), équilibre par bandes, espace stéréo, densité d'écriture, inspection MIDI, câbles morts d'un preset hors `public/` (`check-preset-file.mjs`). Aucun script ne contient de note d'un morceau ; les références restent dans `target/`. Le README liste aussi les leçons du banc (une mesure = garde-fou, jamais une cible) et les particularités du moteur rencontrées (ADSR sans Decay/Sustain, un oscillateur = une voix, tempo propre du `midi-file-sequencer`, mixers ÷√N). |
| `scripts/touches-noires.mjs` | `node scripts/touches-noires.mjs` (écrit dans `target/`) · `--public` pour écrire dans l'app | **Touches noires** (preset `touches-noires`, groupe Showcase) : lead polyphonique qui ronronne, nappe à trémolo, nappe haute « la question », accords pincés, basse, batterie fabriquée en sinus et bruit ; 24 mesures, nos propres notes sur les cinq touches noires. Les instruments viennent de **`scripts/kit/touches-noires-kit.mjs`** : `buildKit({ bpm, ticksPerBeat, totalTicks, parts })` rend un graphe à partir de listes de notes (lead, basse, 3 voix de nappe, 4 voix d'accords pincés, kick/clac/charley) et ne contient AUCUNE note ; il exporte aussi les aides d'arrangement (`mergeRuns`, `lanes`, `byRole`, `hold`, `arrange`, `deal`, `line`). Un preset personnel piloté par un MIDI se régénère avec un petit script local qui importe ce kit et écrit dans `public/presets/local/` (gitignoré, listé dans l'app via `presets/local/manifest.json`). Ce dossier local n'étant sauvegardé nulle part, son contenu utile (preset, manifest, script de régénération, MIDI, relevés) est archivé dans **`etude-locale.7z`** à la racine : 7z AES-256 à noms de fichiers chiffrés, mot de passe connu du seul propriétaire du dépôt — restauration : `7z x etude-locale.7z` à la racine puis `node public/presets/local/etude-fingerbib-sources/regenerer.mjs`. À refaire à la main quand l'étude change (commande dans le `LISEZMOI.txt` du dossier local). ⚠️ 118 modules / 4 `midi-file-sequencer` : le kit contourne l'ADSR sans Decay/Sustain, la monophonie d'un oscillateur et le tempo propre du séquenceur MIDI (voir `scripts/bench/README.md`). |
| `scripts/touches-noires-midi.mjs` | `node scripts/touches-noires-midi.mjs <morceau.mid>` · `--inspect` · `--lead 3+24 --pad 0 --bass 4 --kick 1@27 …` · `<fiche.json>` · `--install` | **Le kit Touches noires joue n'importe quel fichier MIDI** (mode d'emploi : `scripts/kit/README.md`). Répartit les pistes sur les instruments du kit : un rôle = `pistes[@hauteurs]` (`0,5` deux pistes · `3+24` transposée · `1@35,36` seulement ces notes) ; sans rôle donné la répartition est DEVINÉE (batterie General MIDI, basse = la plus grave, lead = la ligne la plus haute, reste en nappe/accords pincés, chaque piste ramenée par octaves dans le registre de son instrument) et affichée comme la ligne de commande équivalente. Lead polyphonique → 4 voix (`deal`) + ombre sur la note du dessus (`line`). Le niveau de sortie est calibré par un rendu hors ligne du morceau entier (`render_graph`, crête 0,95). Écrit `target/<id>.json` (bouton Import) + `target/<id>-flat.json` (banc) ; `--install` écrit dans `public/presets/local/` + son manifest (groupe « Local », hors dépôt ; ⚠️ Vite recharge la page). Ne contient aucune note ; option ou clé inconnue = erreur, jamais ignorée en silence. |
