...
### UI / UX
- [ ] **SONG mode (arrangement timeline)** - IMPLÉMENTÉ (plan v1+v2 complet) sur la branche `feat/song-mode` (2026-07), pas encore mergé. Livré : vue SONG (sections, lanes MIX on/off + courbes de volume, lanes ♪ NOTES + piano-roll avec bande vélocité, ▦ patterns batterie A/B/FILL avec capture, ⚙ AUTOMATION de n'importe quel param (non destructif, moteur seulement), transfert step-seq→clip, undo local ↶/↷, règle de seek via `set_transport_beats` moteur), mode de lecture RACK|SONG global dans le transport, persistance dans les projets v2, 7 projets démo « Songs » (Studio Song, NOVA ⚡/II/III/64/ÆTERNA, DUO). Implémentation → `docs/FEATURES.md` § SONG Mode. Restes (phase 3) → `docs/SONG_MODE_PLAN.md` : recorder cv/gate (génératifs), Ctrl+Z contextuel, loi log fréquences. Audit séquenceurs : `docs/STUDIO_GAP_ANALYSIS.md` (sur main).
