...
| Kits de batterie réglés « au hasard » (paramètres morts + valeurs hors plage) | Même cause racine que les câbles morts, côté params : `attack: 50` sur un knob 0–1, `tune: 8000` sur un knob ×0.5–2 (le DSP `clamp()` en silence), `909-kick.level` qui n'existe pas, et `compressor-drums` dont le motif dormait dans un param `steps` inconnu (le moteur lit `drumData`) avec une horloge en `bpm` (il lit `tempo`) → preset qui charge, joue… sans batterie | 6 presets corrigés après A/B à l'oreille + nouveau garde-fou `npm run check:preset-params` (noms extraits du Rust, plages extraites de la table des knobs), branché sur la CI. Reste 127 cas antérieurs en baseline (`scripts/check-preset-params-baseline.json`) |
