...
### UI / UX
- [ ] **SONG mode (arrangement timeline)** - IMPLÉMENTÉ (plan v1+v2 complet + phase 3) sur la branche `feat/song-mode` (2026-07), pas encore mergé. Livré : vue SONG (sections, lanes MIX on/off + courbes de volume, lanes ♪ NOTES + piano-roll avec bande vélocité, ▦ patterns batterie A/B/FILL avec capture, ⚙ AUTOMATION de n'importe quel param (non destructif, moteur seulement, **loi LIN|LOG** — LOG auto sur freq/cutoff), transferts statiques step-seq/chord/polyrhythm/euclidean→clip, **⏺ REC moteur des génératifs** (arpeggiator/turing/gravity — recorder cv/gate `crates/dsp-graph/src/recorder.rs`, 4 couches + Tauri), undo local ↶/↷, Ctrl+Z contextuel, règle de seek via `set_transport_beats` moteur), mode de lecture RACK|SONG global dans le transport, persistance dans les projets v2, 7 projets démo « Songs » (Studio Song, NOVA ⚡/II/III/64/ÆTERNA, DUO). Implémentation → `docs/FEATURES.md` § SONG Mode. Audit séquenceurs : `docs/STUDIO_GAP_ANALYSIS.md` (sur main).
